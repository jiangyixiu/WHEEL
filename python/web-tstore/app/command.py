...
def refresh_createvolume_status(volume_name='all'):
    success, result, volume_stop_all = execute_volume_status(volume_name)
    if success:
        volume_running = list()
...
def refresh_volume_status(volume_name='all'):
    ...
    if success:
        ...
        if len(result):
            for volume in result:
                ...
                for brick in volume['bricks']:
                    ...
                    l_index = brick_name.find(' ')
                    address = brick_name[l_index:].strip()
                    if 'Total Disk Space' in brick.keys():
                        totalSpace = brick["Total Disk Space"]
                        if len(totalSpace) > 20:
                            total = -1
                        else:
                            if totalSpace == '0Bytes':
                                total = -1
                            else:
                                total = float(brick['Total Disk Space'][:-2])
                    else:
                        total = -1
                    if 'Disk Space Free' in brick.keys():
                        diskFree = brick["Disk Space Free"]
                        if len(diskFree) > 20:
                            free = -1
                        else:
                            if diskFree == '0Bytes':
        ...
        if len(volume_stop_all):
            for volume in volume_stop_all:
        ...
        volume_deleted = list((set(volume_disappear) | set(volume_stop_all)) -
                              (set(volume_disappear) & set(volume_stop_all)))
...
def refresh_disk_io():
    ...
    for machine in cluster_list:
        ...
        Redis.rpush(DISK_NAME_WRITE + machine, {DATA: sum_name_write_throughtput, TIME: timeStamp})
        ...
        Redis.rpush(DISK_NAME_READ + machine, {DATA: sum_name_read_throughput, TIME: timeStamp})
    i = 0
    sum_writes = 0
    sum_reads = 0
    ...
    while i < len(sum_diskio_writes):
        ...
        i += 1
    for j in sum_diskio_write:
        sum_writes += j
    for k in sum_diskio_read:
        sum_reads += k
    Redis.rpush(DISKWRITEALL, {DATA: sum_writes, TIME: timeStamp})
    Redis.rpush(DISKREADALL, {DATA: sum_reads, TIME: timeStamp})


def network_io(hostname):
    networkIO = snmp_query('ifName', hostname, 'public')
    # logging.debug(networkIO)
    if len(networkIO) < 1:
        return "0", "0"
...
def refresh_network_io():
    ...
    for machine in cluster_list:
        ...
        Redis.rpush(NETWORKIO_NAME_IN_INIT + machine, {DATA: network_io_in_speed_throughput, TIME: timeStamp})
        Redis.rpush(NETWORKIO_NAME_OUT_INIT + machine, {DATA: network_io_out_speed_throughput, TIME: timeStamp})
    ...
    Redis.rpush(NETWORKIO_IN_SUM_INIT, {DATA: sum_network_in_speed, TIME: timeStamp})
    Redis.rpush(NETWORKIO_OUT_SUM_INIT, {DATA: sum_network_out_speed, TIME: timeStamp})
...
def monitor_resource(hostname):
    ...
    # logging.debug(len(storage_size))
    # logging.debug(len(storage_used))
    ...
    while i < len_storage_descr:
        ...
        # logging.debug(hostname+":"+ storage_used[i])
...
def refresh_machine_resource():
    ...
    for machine in cluster_list:
        ...
        Redis.rpush(MEMORY_USAGE_PREFIX + machine, memory_usage)
        i = 0
        while i < len(resource['cpus']):
            Redis.rpush(CPU_USAGE_PREFIX + machine + ':' + str(i), cpu_usages[i])
            i += 1
...
def query_machine_resource_local():
    ...
    Redis.rpush(MEMORY_USAGE_PREFIX + CONFIG_LOCAL_HOST, memory_usage)
    i = 0
    while i < len(resource_dict[CONFIG_LOCAL_HOST]['cpus']):
        Redis.rpush(CPU_USAGE_PREFIX + CONFIG_LOCAL_HOST + ':' + str(i), cpu_usages[i])
        i += 1
...
def query_volume_perf():
    ...
    for volume_name in volume_names:
        ...
        if success:
            Redis.rpush(READ_SPEED_PREFIX + volume_name, read_perf)
        success, write_perf = volume_perf(volume_name, False)
        if success:
            Redis.rpush(WRITE_SPEED_PREFIX + volume_name, write_perf)
