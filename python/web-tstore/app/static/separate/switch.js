...
//      		$('body').removeClass().addClass(tmp_lang);
	    ...
	    setTimeout(function(){ 
			var dafal_lang='zh';
		    language_pack.loadProperties(dafal_lang);
			...
			$(".lang_list").click(function(){
				...
				$('.rel').html(checked);
				language_pack.loadProperties(icon);  
				record_lang=icon;
			})				
			var monitorNavs = $("#monitorMNav").find("li");
	    	monitorNavs.on('click', function (e) {
				language_pack.loadProperties(record_lang);   
				e.preventDefault;
			});
	    },1500);    
//		}); 
