$(document).ready(function(){
	...
	var monitor_info;
	$.when(monitorInfo()).done(function(data){
		monitor_info = data;
    ...
    $.when(getServerStatusInfo()).done(function(data){
...
//		    },
  ...
  //                text: '内存使用率(%, '+getMemsize(COUNT)+'G)'
	    ...
	    $('div[id^="CPU"] .highcharts-axis.highcharts-yaxis text').attr("data-lang","y_cpu");
    	$('div[id^="Mem"] .highcharts-axis.highcharts-yaxis text').attr("data-lang","y_mem");								
		
		$('div[id^="RW"] .highcharts-legend-item.highcharts-line-series:eq(0) text').attr('data-lang','read');
		$('div[id^="RW"] .highcharts-legend-item.highcharts-line-series:eq(1) text').attr('data-lang','write');
		$('div[id^="NETWORK"] .highcharts-legend-item.highcharts-line-series:eq(0) text').attr('data-lang','in');
		$('div[id^="NETWORK"] .highcharts-legend-item.highcharts-line-series:eq(1) text').attr('data-lang','out');
        ...
        for (var i = 0; i < serversinfo[COUNT]["disks"].length; i++) {
	        ...
	        $('#monitorPieChart'+COUNT+"disk"+ i).highcharts({
	            ...
	            title: {
	              text: serversinfo[COUNT]["disks"][i]["name"]
	            },
	            tooltip: {
	        ...
	        $('.pieChartShow:eq('+i+') text:eq(1)').attr("data-lang","used");
			$('.pieChartShow:eq('+i+') text:eq(2)').attr("data-lang","not_used");
	    }//disk循环结束
	    Highcharts.setOptions({
	        colors: ['#058DC7', '#50B432', '#ED561B', '#DDDF00', '#24CBE5', '#64E572', '#FF9655', '#FFF263', '#6AF9C4']
	    });
    ...
    function monitorinfoFormat(serverId, i) {
	    return '<div class="col-md-12">'+
	            ...
	            '<h3 class ="box-title" id="serverid"><span data-lang="machine_name"></span>:' + serverId + '</h3>' +
	            '</div>' +
	            '<div class= "col-md-6">' +
	            '<div class="box box-primary">' +
	            '<div class="box-header with-border">' +
	            '<i class="fa fa-bar-chart-o"></i>' +
	            '<h3 class="box-title" data-lang="cpu"></h3>' +
	            '</div>' +
	            ...
	            '<div class="box-header with-border">' +
	            '<i class="fa fa-bar-chart-o"></i>' +
	            '<h3 class="box-title" data-lang="memory_usage"></h3>' +
	            '</div>' +
	            ...
	            '<i class="fa fa-bar-chart-o"></i>'+
	            '<h3 class="box-title" data-lang="disk_io"></h3>'+
	            '</div>'+
	            ...
	            '<div class="col-md-12"><div class="box box-primary"><div class="box-header with-border">'+
	            '<i class="fa fa-bar-chart-o"></i>'+
	            '<h3 class="box-title" data-lang="network_io"></h3>'+
	            '</div>'+
	            ...
	            '<div class="box-header with-border">' +
	            '<i class="fa fa-bar-chart-o"></i>' +
	            '<h3 class="box-title" data-lang="disk_usage"></h3>' +
	            '</div>' +
