$(document).ready(function() {
    ...
    var volumeData;
    $.when(get_volumedata()).done(function(data){ 
    	volumeData = data;
	    ...
	    function getSysCapacity() {
	        ...
	        $.getJSON(url, null, function(data) {
			    ...
			    for (var i = 0; i < volumeData.length; i++) {
			        ...
			        tmp["capacity"] = volumeData[i]["capacity"] + "GB";
			        if (volumeData[i]["nfs"]) {
			    ...
			    $('[data-field="volumeName"] div:eq(0)').attr('data-lang','volume_name');
			    $('[data-field="status"] div:eq(0)').attr('data-lang','status');
			    $('[data-field="capacity"] div:eq(0)').attr('data-lang','capacity');
			      //使用与未使用切换
...
//	        }
    ...
    $('#sum_diskio_speed').highcharts('StockChart', {
        ...
        series: [{
            name: 'Read',
            data: (function() {
                var valueTime = clusterInfo["init_disk_read_sum"][20];
                var JsonData = eval('(' + valueTime + ')');
                var data = [],
                    time = parseInt(JsonData.time),
                    j;
                for (j = -359; j < 0; j += 1) {
                    var valueTime = clusterInfo["init_disk_read_sum"][j + 359];
                    var JsonData = eval('(' + valueTime + ')');
                    data.push([
                        time + j * 1000,
...
//              	console.log($(".tipRead"));
	...
	$(".highcharts-legend-item text[text-anchor='start']:eq(0)").attr('data-lang','read');
	$(".highcharts-legend-item text[text-anchor='start']:eq(1)").attr('data-lang','write');
	$(".highcharts-legend-item text[text-anchor='start']:eq(2)").attr('data-lang','in');
	$(".highcharts-legend-item text[text-anchor='start']:eq(3)").attr('data-lang','out');
