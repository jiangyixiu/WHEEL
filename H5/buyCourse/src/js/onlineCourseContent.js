...
var key = request('key');//分销
if (from=='2017062011'&& courseId=='848') {
    window.location.href = '../onlineCourseContent.html';
}
$(function(){
                ...
                success: function (data) {
                    if (data.data.isOver == '1'||courseId == '765') {
                        ...
                        if (!key) {
                            ...
                            var url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' + appid + '&redirect_uri=' + redirect_uri + courseId + encodeURIComponent("?from=" + from + "&key=" + key) + '&response_type=code&scope=snsapi_userinfo&state=123#wechat_redirect';
                        };
                        //console.log(url)
