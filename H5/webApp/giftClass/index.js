...
function checkHasBuy() {
	...
	$.getJSON(baseUrl + "checkCrowdfunding", {
		...
		var courseType = '0';//线下
		if(data.data == "have1") { // 已购此课
			var url = encodeURI("http://www.chazuomba.com/files/webApp/giftClass/audit.html?audit=" + audit + "&unionId=" + unionId + "&orderCode=" + orderCode + "&openid=" + openid + "&courseId=" + courseId + "&type=0" + "&courseType=" + courseType);
...
function checkRights() {
	...
	$.ajax({
		...
		success: function(data){
			...
			window.location.href = 'giftClassApply.html?free=' + free + '&money=' + Math.floor(price) + '&title=' + $(".title").text() + '&audit=' + audit;
		}
	});
}
//保存用户数据
