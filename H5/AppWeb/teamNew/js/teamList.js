...
//	$scope.nextPage = 1;
	...
	//到培训详情页
	$scope.toMsg = function(catalogId, companyCultivateId, name){
		_czc.push(["_trackEvent", "1", "排行榜", "团队"]);
		var url = 'ranking.html?companyId=' + companyId + '&catalogId=' + catalogId + '&companyCultivateId=' + companyCultivateId + '&name=' + encodeURIComponent(name);
		navigateTo({
		    'title': name,
		    'url':  url
		});
