...
//	$scope.nextPage = 1;
	...
	//到培训详情页
	$scope.toMsg = function(catalogId, companyCultivateId, classSts, company_study_end_at, name){
		//classSts 课程状态：1 进行中、2已完成，
		//已完成课程在详情页展示最后一个月数据，其他展示当月数据
		_czc.push(["_trackEvent", "1", "培训情况", "管理员"]);
		var url = 'trainingMsg.html?companyId=' + companyId + '&catalogId=' + catalogId + '&companyCultivateId=' + companyCultivateId + '&name=' + encodeURIComponent(name);
		if(classSts == '2'){
			var endtime = Date.parse(company_study_end_at.replace(/-/g,"/"));
			var date = new Date(endtime);
			var year = date.getFullYear();
			var month = date.getMonth() + 1;
			month = (month > 9) ? month : ('0' + month);
			url += '&endyear=' + year + '&endmonth=' + month;
		}
		navigateTo({
		    'title': '每日团队数据',
		    'bounces': false,
