...
// 获取图片资源
request(avatar)
    .pipe(fs.createWriteStream('./public/poster/avatar.png')
        .on('close', () => {
            qr_png()
        }));
...
function qr_png() {
    qr.image(url, {
        ...
        margin: 2,
        size: 3
    }).pipe(fs.createWriteStream("public/poster/qr_course.png")
        .on('close', () => {
            im("public/images/poster_bg.png")
                .size(750)      //等比缩放图像到750像素宽
                .draw(im("public/poster/avatar.png").size(80), 335, 390)   // 在(x,y)处绘制
                .draw(im("public/poster/qr_course.png").size(155), 150, 1050)
                .save("public/poster/output.jpg", {
...
function upload() {
    ...
    var localFile = "./public/poster/poster.png";
